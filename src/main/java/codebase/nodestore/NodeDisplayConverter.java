 ...
 * </ol>
 */
public class NodeDisplayConverter implements Converter {
    ...
    private void checkAndSkip(DataInput input, char expected) throws IOException {
        ...
        if (ch != expected) {
            throw new IOException(
                    "Malformed line: found '" + ch + "' while expecting '" + expected + "'");
        }
    }
    ...
    private void checkAndSkip(DataInput input, String expected) throws IOException {
        for (int i = 0; i < expected.length(); i++) {
            ...
            if (ch != expected.charAt(i)) {
                throw new IOException(
                        "Malformed line: found '" + ch + "' while expecting of '" + expected + "'");
            }
        }
    }
    ...
    private void skipUntilChar(DataInput input, char testChar) throws IOException {
        while (true) {
            /*
             * Either the char is found or the loop is interrupted with an EOFException
             */
    ...
    public void write(DataOutput dataOutput, Object object) throws IOException {
        ...
        if (object instanceof AttributeNode) {
            ...
            nodeRepresentation = "#" + node.getNumber() + " = I("
                    + codebase.StringUtil.stringify(node.getTypeName()) + ")";
        } else if (object instanceof RelationNode) {
            final RelationNode node = (RelationNode) object;
            nodeRepresentation =
                "#" + node.getNumber() + " = R(" + codebase.StringUtil.stringify(node.getTypeName())
        ...
        dataOutput.write(nodeRepresentation.getBytes(StringUtil.UTF8));
