...
/**
 * Utility class that simplifies AES ciphering and deciphering operations.
 * <p>
 * The first step is to create the cipher key and the IV parameter. This can be created
 * from a byte[] (with correct size for an AES key in a multiple on 16 bytes) and then
 * call {@link #getKey(byte[])} and {@link #getIv(byte[])}.
 * <p>
 * The main methods are to be called for ciphering and deciphering are, respectively,
 ...
 * message will be padded if the message size does not end on a 16-byte boundary.
 * 
...
public final class AESUtil {
    ...
    public static String decypherText(Key key, IvParameterSpec iv, String cyphertext)
        ...
        return StringUtil.UTF8.decode(bb).toString();
    }
