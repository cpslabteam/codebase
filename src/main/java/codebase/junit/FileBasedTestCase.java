...
public abstract class FileBasedTestCase extends TestCase {
    ...
    protected void assertEqualContent(final File f0, final File f1) {
        try {
            ...
            throw new RuntimeException(
                    "IO exception while comparing " + f0.getName() + " with " + f1.getName(), e);
        }
    }
    ...
    protected void assertEqualContent(final File file, final byte[] b0) {
        try {
            ...
            try {
                ...
                assertTrue("Different number of bytes",
                        numRead == b0.length && is.available() == 0);
    ...
    protected void assertEqualContent(final char[] c0, final File file) {
        try {
            final Reader ir =
                new java.io.InputStreamReader(new FileInputStream(file), StringUtil.UTF8);
            try {
