...
public class Format {
     ...
     * <p>
     * This routine is means to be used in to dump strings that may have
     * binary data.
     * 
     * @param string the string to be converted
     * @param invisible the character to replace the invisible control
     *            characters
     * @return a string where invisible characters are replaced by a
    ...
    public static long parseBytes(final String s) {
        ...
        if (u.endsWith(Conversions.GB_UNIT)) {
            bytes = Conversions.gbToBytes(Double.parseDouble(u.substring(0, u
                .length()
                    - Conversions.GB_UNIT.length())));
        } else if (u.endsWith(Conversions.MB_UNIT)) {
            bytes = Conversions.mbToBytes(Double.parseDouble(u.substring(0, u
                .length()
                    - Conversions.MB_UNIT.length())));
        } else if (u.endsWith(Conversions.KB_UNIT)) {
            bytes = Conversions.kbToBytes(Double.parseDouble(u.substring(0, u
                .length()
                    - Conversions.KB_UNIT.length())));
        } else if (u.endsWith(Conversions.BYTE_UNIT)) {
            bytes = (long) java.lang.Math.ceil(Double.parseDouble(u
                .substring(0, u.length() - Conversions.BYTE_UNIT.length())));
        } else {
    ...
    public static String formatBytes(final long bytes, final int decimals) {
        ...
        if (bytes >= Conversions.GB) {
            result = Double.toString(Math.round(Conversions.gb(bytes),
                decimals))
                    + Conversions.GB_UNIT;
        } else if (bytes >= Conversions.MB) {
            result = Double.toString(Math.round(Conversions.mb(bytes),
                decimals))
                    + Conversions.MB_UNIT;
        } else if (bytes >= Conversions.KB) {
            result = Double.toString(Math.round(Conversions.kb(bytes),
                decimals))
                    + Conversions.KB_UNIT;
        } else {
