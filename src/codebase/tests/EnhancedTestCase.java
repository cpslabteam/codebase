 ...
 * <p>
 * Without this primitives the code for <tt>junit</tt> tests cases becomes much
 * more difficult to write and maintain. When a test fails one needs contextual
 * information about what is failing. This information can only be delivered by
 * using assertion that know the domain.
 * <p>
...
public class EnhancedTestCase
    ...
    /**
     * A private class used for matching the elements of the expected iterator
     * with the elements of the actual iterator. In order to compare to
     * iterators we need to count the number of occurrences of similar elements.
     * This class wraps an object with a number two reference counters. The
     * number of times the object is seen on the expected iterators of times it
     * is then seen on the actual iterator. If these reference counters are not
     * the same this means that The number of occurrences of the object in the
     * expected and in the actual iterator do not match.
     */
    private static class RefObject
            extends Object {
        /**
        ...
        /**
         * Checks if are equal to another object. It compares the underling base
         * elements but not through the java equals() method instead is uses the
         * Debug.equals() method wich is capable of handeling arrays.
         * 
         * @param o another entry to compare to
         * @return <code>true</code> if the underling base elements have the
         *         same content.
         */
        ...
        /**
         * Informs that this object was found in the expected iterator once
         * more.
         */
        ...
        /**
         * @return the number of references on the expected iterator
         */
        public int getExpectedRefCount() {
            return expectedRefCount;
        }
        ...
        /**
         * @return the number of references on the actual iterator
         */
        public int getActualRefCount() {
            return actualRefCount;
        }

        /**
         * Check is the number of expected references matches the number of
         * actual references.
         * 
         * @return <code>true</code> if the number of expected references
         *         matches the number of actual references. <code>false</code>
         *         otherwise.
         */
        public boolean isOk() {
         ...
         * 
         * @return the base object element toghether with the expected and
         *         actual numbers of references.
         */
    ...
    /**
     * Checks that two iterators are equal using a bag semantics. In other words
     * it compares the elements of two iterators and furthermore it counts the
     * number of ocurrences of each element.
     * 
     ...
     */
    public static final void assertBagEquals(final Iterator expected,
            final Iterator actual) {
        performBaseNullChecks(expected, actual);
        performIteratorEmptyChecks(expected, actual);

        final java.util.List expectedElements = makeExpectedElems(expected);
     ...
     * <p>
     * When an invisible char is displayed, it can cause undesired effects on
     * the tty.
     * 
     ...
     * <p>
     * This method is intended to serve as a replacement for JUnit's assert when
     * comparing two chars. JUnit reports a difference between to integer
     * values, which is not very helpful.
     * 
    ...
    public static final void assertEquals(final char expected, final char actual) {
        if (expected != actual) {
            throw new junit.framework.AssertionFailedError("Expecting <"
                    + safeChar(expected) + "> (" + (int) expected + ", 0x"
                    + Binary.toHex(expected) + ") but found <" + safeChar(actual) + "> ("
                    + (int) actual + ", 0x" + Binary.toHex(actual) + ").");
        }
    }
     ...
     * <p>
     * In a bag allows multiple occurrences of the the same element. The
     * comparison using bag semantics does not make any assumptions on ordering.
     * It is only required that number of copies of the elements are the same in
     * both operands.
     * 
     * @param expected the array of expected elements
     * @param actual the array of actual elements
     */
    public static final void assertBagEquals(final Object[] expected,
            final Object[] actual) {
        performBaseNullChecks(expected, actual);
        performArrayEmptyChecks(expected, actual);

        final Iterator expectedIterator = new ArrayIterator(expected);
        final Iterator actualIterator = new ArrayIterator(actual);
        final java.util.List expectedElements = makeExpectedElems(expectedIterator);
     ...
     * @param actual the iterator with the elements that should be contained in
     *            the iterator of expected elements.
     */
    public static final void assertIsContained(final Iterator expected,
            final Iterator actual) {
        performBaseNullChecks(expected, actual);
        performIteratorEmptyChecks(expected, actual);
     ...
     * <p>
     * If the code block does not throw any exception the method fails. If the
     * code block throws an exception of a different type (types a checked by
     * comparing the class names literally), the method also fails.
     * 
     * @param block the block of code to be called
     * @param throwable an instance of the exception class
     * @throws junit.framework.AssertionFailedError if the code throws an
     *             exception of a different type of did not throw any exception
     */
        ...
        if (exception != null) {
            if (exception.getClass() != throwable.getClass()) {
                fail("Got exception " + exception.getMessage() + "of type "
                        ...
                        + ", but expecting an exception of type "
                        + throwable.getClass().getName());
            }
        } else {
            fail("Expected exception of type " + throwable.getClass().getName()
                    + " did not occur");
        }
    };
     ...
     * <p>
     * Uses the {@link RefObject} class in order to maintain the expected and
     * actual numbers of references. This list will be treated by other methods.
     * 
     * @param iterator an iterator in the expected objects
     * @return a list of RefObject elements
     */
    private static java.util.List makeExpectedElems(final java.util.Iterator iterator) {
        /*
         * Adds all elements to the list
         */
        final java.util.List expectedElements = new java.util.ArrayList();
     ...
     * @return the object with the corresponding count of references or
     *         <code>null</code> if not found
     */
    private static RefObject getRefObjectContaining(final List refObjectList,
            final Object element) {
        for (final Iterator it = refObjectList.iterator(); it.hasNext();) {
    ...
    /**
     * Checks that all the elements seen in the actual iterator are expected.
     * For each actual element seen it also increments the number of actual
     * references.
     * 
     * @param expectedElements the list with reference count information on
     *            objects
        ...
        while (actualIterator.hasNext()) {
            ...
            if (elemIndex >= 0) {
                ...
                throw new junit.framework.AssertionFailedError("The element <"
                        ...
                        + " was not present in the expected iterator");
            }
        }
    }

    /**
     * Verifies that the expected and actual numbers of references of each
     * element in the reference count list are equal. I.e., that they match up
     * to bag semantics.
     * 
     * @param expectedElements the list with reference count information on
     *            objects
     */
    private static void performBagReferenceCheck(final java.util.List expectedElements) {
        for (final java.util.Iterator it = expectedElements.listIterator(); it.hasNext();) {
            final RefObject entry = (RefObject) it.next();
            if (!entry.isOk()) {
                throw new junit.framework.AssertionFailedError(
                        "Mismatched number of occourrences on the "
                                + "expected and actual iterators. "
                                + Debug.toString(entry));
            }
        }
    }

    /**
     * Compares two rows throwing the appropriate exception in case the elements
     * of the columns do not match.
     * <p>
     * If the elements are arrays we perform a recursive element by element
     * comparison (because we may have arrays of arrays). Otherwise we use the
     * <code>equals</code> method. If this method fails we check that the string
     * representation is the same.
     * <p>
     * <b>NOTE: </b> This is very important because some objects are very
     * difficult to compare recursively.
     * 
     ...
     */
    private static void compareRows(final int rowNum, final Object expectedValue,
     ...
     * @throws junit.framework.AssertionFailedError if only one of the arguments
     *             is null
     */
    ...
    /**
     * Checks that two objects arrays are equal. It compares the length of the
     * arrays and then it performs a positional comparison of each element of
     * the array using the method <code>equals()</code>
     * 
        ...
        if (expectedArray.length != actualArray.length) {
            throw new junit.framework.AssertionFailedError(
                    "Actual and expected do not have the same length: length(expected)="
                            + Integer.toString(expectedArray.length)
                            + ", length(actual)=" + Integer.toString(actualArray.length));
        }
    ...
    /**
     * Verifies that either the two object arrays have elements or that they
     * both are empty.
     * 
     * @param actual the actual array
     * @param expected the expected array that represents the expected result of
     *            the test
     * @throws junit.framework.AssertionFailedError if only one of the arguments
     *             is null
     */
    private static void performArrayEmptyChecks(final Object[] expected,
            final Object[] actual) {
    ...
    /**
     * Verifies that either the two iterators have elements or that they both do
     * not have more elements. Note that this is not the same as being empty
     * because the elements may already have been exhausted fro the iterators.
     * 
     * @param actual the actual iterator
    ...
    private static void performIsContainedCheck(final Iterator expected,
            final Iterator actual) {
        final List expectedElems = new ArrayList();
        ...
        while (actual.hasNext()) {
            ...
            if (!expectedElems.contains(elem)) {
                throw new junit.framework.AssertionFailedError(
                        "Actual element at (0-based) position "
                                + Integer.toString(rowNum)
    ...
    /**
     * Checks that a result is near the expected result. In other words checks
     * that <code>abs(expected - actual) <= error</code>.
     * 
     * @param expected the expected values
     * @param actual the actual value
     * @param error the error interval
     */
    public static final void assertDelta(final double expected, final double actual,
            final double error) {
    ...
    public final void assertEqualResults(final Object expected, final Object[] actual) {
        ...
        if (actual.length > 1) {
            throw new junit.framework.AssertionFailedError(
                    "The function result has many values <" + Debug.toString(actual)
                            + ">. Expecting the singleton element <"
                            + Debug.toString(expected) + ">.");
        }
        ...
        if (!result.equals(expected)) {
            throw new junit.framework.AssertionFailedError(
                    "The function returned value <" + Debug.toString(result)
