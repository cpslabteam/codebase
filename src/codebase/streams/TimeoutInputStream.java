...
public class TimeoutInputStream extends
    ...
    public TimeoutInputStream(final InputStream in, final int timeout, TimeUnit timeoutUnit) {
        ...
        timeoutProperty =
            new Property.PropertyBuilder("Timeout", new NumberDataType()).setCloneable(true).setName("Stream Timeout")
                    .setReadOnly(false).setTransient(true).setValueStore(new DefaultValueStore(timeout))
     ...
     * @param in the input stream to be decorated from where the reading will take place.
     * @param timeout the driver timeout parameter for open, read and write operations in
     *            millis. Must be a {@link NumberDataType}. This stream will be attached
     *            to this property and each update in the property's value will instantly
     *            affect the stream
     * @param timeoutUnit the units of the timeout parameter
     */
