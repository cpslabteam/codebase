 ...
 * <tt>Double</tt> objects. In addition to the read and write methods that
 * read or write <tt>Double</tt> objects this class contains readDouble and
 * writeDouble methods that convert the <tt>Double</tt> object after reading
 * or before writing it to its primitive <tt>double</tt> type.
 * <p>
 * Example usage (1).
 * 
 * <pre>
 ...
 *     
 *     DoubleConverter.DEFAULT_INSTANCE.write(new DataOutputStream(output),
 *         new Double(2.7236512));
 *     DoubleConverter.DEFAULT_INSTANCE.writeDouble(new java.io.DataOutputStream(
 *         output), 6.123853d);
 *     
 ...
 *     
 *     double d1 = DoubleConverter.DEFAULT_INSTANCE
 *         .readDouble(new DataInputStream(input));
 *     Double d2 = (Double) DoubleConverter.DEFAULT_INSTANCE
...
public class DoubleConverter
     ...
     * <tt>double</tt> value of a <tt>Double</tt> object. Because this size
     ...
     * <tt>Double</tt> object. So it does not matter when the specified object
     * is <tt>null</tt>.
     * 
     * @param dataInput the stream to read the <tt>double</tt> value from in
     *            order to return a <tt>Double</tt> object.
     ...
     * This implementation calls the writeDouble method of the data output with
     * the <tt>double</tt> value of the object.
     * 
     * @param dataOutput the stream to write the <tt>double</tt> value of the
     *            specified <tt>Double</tt> object to.
     ...
     * output. <br>
     * This implementation calls the write method with a <tt>Double</tt>
     * object wrapping the specified <tt>double</tt> value.
     * 
     * @param dataOutput the stream to write the specified <tt>double</tt>
     *            value to.
     * @param d the <tt>double</tt> value that should be written to the data
     *            output.
     * @throws IOException includes any I/O exceptions that may occur.
     */
    ...
    /**
     * The main method contains some examples how to use a DoubleConverter. It
     * can also be used to test the functionality of a DoubleConverter.
     * 
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // write a Double and a double value to the output stream
            new DoubleConverter().write(
                new java.io.DataOutputStream(output), new Double(2.7236512));
            new DoubleConverter().writeDouble(
                new java.io.DataOutputStream(output), 6.123853d);
            // create a byte array input stream on the output stream
            java.io.ByteArrayInputStream input = new java.io.ByteArrayInputStream(
                output.toByteArray());
            // read a double value and a Double from the input stream
            double d1 = new DoubleConverter()
                .readDouble(new java.io.DataInputStream(input));
            Double d2 = (Double) new DoubleConverter()
