 ...
 * <p>
 * Strings handled by this converter can be ascii or ebcdic as long as each
 * character is only one byte. represented as an array of ASCII characters.
 * <p>
 * The strings are only delimited by the size specified in the constructor. When
 * reading, only a fixed number of characters are considered. No end of string
 * marker is sought. When writing only a predefined number of characters are
 * written out. No end of line marker is written. If the String is smaller that
 * the fixed sized, then it is right padded with space characters.
 */
public class FixedSizeStringConverter
	 ...
	 * <p>
	 * Descending classes can override this method to provide a different
	 * encoding.
	 * 
