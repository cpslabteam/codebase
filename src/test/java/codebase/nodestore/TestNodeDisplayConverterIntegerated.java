...
public class TestNodeDisplayConverterIntegerated extends TestCase {

    public static final String nodeStore =
        "#1 = I(\"type1\")\n" + "#2 = I(\"type2\")\n" + "#3 = R(\"child\", 1, 2)\n";
    ...
    public void testNodeInput() throws IOException, ClassNotFoundException {
        ...
        ObjectConverterInput nodeInput =
            new ObjectConverterInput(inputStream, new NodeDisplayConverter());

        // create a node output
        byte[] buffer = new byte[255];
        DataOutput output = new ByteArrayDataOutput(buffer);
        ObjectConverterOutput nodeOutput =
            new ObjectConverterOutput(output, new NodeDisplayConverter());
